  ...
  for result in results: 
    if result[3] == None: 
      if result[4] in ['AF', 'FL', 'UF']: 
        print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}   fail")
      elif result[4] in ['AS','AW','PW','RD','NF','LE','PE','WD','WJ']: 
        print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}")
      else: 
        print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}  {result[5]:2d}uoc")
    else: 
      if result[4] in ['AF', 'FL', 'UF']: 
        print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}   fail")
      elif result[4] in ['AS','AW','PW','RD','NF','LE','PE','WD','WJ']: 
        print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}")
      else: 
        print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}  {result[5]:2d}uoc")
...
def print_courses(cur, course_list): 
  ...
  for course in course_list: 
    if course[0] == "{": 
      ...
      cur.execute(query_courses, [altern[0]])
      name0 = cur.fetchone()
      cur.execute(query_courses, [altern[1]])
      name1 = cur.fetchone()
      print(f"- {altern[0]} ???") if name0 == None else print(f"- {altern[0]} {name0[0]}")
      print(f"  or {altern[1]} ???") if name1 == None else print(f"  or {altern[1]} {name1[0]}")
    else: 
...
def print_completed(cur, zid, strmCode, progCode):
  ...
  for result in results: 
    ...
    if result[4] == "FL": 
      ...
      if result[4] == None: 
        mark = " -"
        print(f"{result[0]} {result[1]} {result[2]:<32s}  - {mark:2s}   0uoc does not satisfy any rule")
        valid_uoc  = 0
      else: 
        ...
        if rule == None: # if it is stil none 
          ...
          if stream_rule == None: 
            ...
            stream_rule = cur.fetchone()  
          if stream_rule == None: # still none 
            cur.execute(query_rule1, ["FREE####", strmCode])
            fe = cur.fetchone()
            if fe == None: 
              print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}   0uoc does not satisfy any rule")
              valid_uoc  = 0
              ...
              if fe[2] != None and fe[2] <= completed_uoc: 
                ...
                # check the amount of 
                print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}  {result[5]:2d}uoc towards Free Electives") 
            ...
            if stream_rule[2] != None and stream_rule[2] <= completed_uoc: 
              ...
              # check the amount of 
              print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}  {result[5]:2d}uoc towards {stream_rule[0]}") 
              completed_courses.append({"code": result[0], "rule":stream_rule[0]})   
        else: 
          completed_courses.append({"code": result[0], "rule":rule[0]})
          print(f"{result[0]} {result[1]} {result[2]:<32s}  - {result[4]:2s}  {result[5]:2d}uoc towards {rule[0]}")
    else: 
      cur.execute(query_rule,[pattern, progCode])
      rule = cur.fetchone()
      ...
      if rule == None: 
        ...
        if stream_rule == None: 
          # check if there is a free elective needed before making it invalid 
            ...
            fe = cur.fetchone()
            if fe == None: 
              print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}   0uoc does not satisfy any rule")
              valid_uoc  = 0
            else: # there is a free elective 
              completed_uoc = course_count(completed_courses, fe[0])*6
              ...
              if fe[2] != None and fe[2] <= completed_uoc: 
                ...
                # check the amount of 
                print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}  {result[5]:2d}uoc towards Free Electives") 
                completed_courses.append({"code": result[0], "rule":fe[0]})  
         
        else: 
          # IT GOES TOWARDS A RULE! - BUT WE NEED TO CHECK IF THEY HAVE ALREADY MAXED OUT ALL THE CREDITS 
          completed_uoc = course_count(completed_courses, stream_rule[0])*6
          #print(stream_rule) 
          if stream_rule[2] != None and stream_rule[2] <= completed_uoc: 
            print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}   0uoc does not satisfy any rule")
            ...
            # check the amount of 
            print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}  {result[5]:2d}uoc towards {stream_rule[0]}") 
            completed_courses.append({"code": result[0], "rule":stream_rule[0]})     
        ...
        print(f"{result[0]} {result[1]} {result[2]:<32s}{result[3]:>3} {result[4]:2s}  {result[5]:2d}uoc towards {rule}")
        completed_courses.append({"code": result[0], "rule": rule})

    if uoc_valid(result[4]) == 1:
      if valid_uoc == 1: 
        total_uoc = total_uoc + result[5]
...
def print_remaining(cur, zid, strmCode, progCode, completed_courses, failed_courses):
  ...
  if free_elective != []: 
    ...
    uoc_remaining = free_elective[1] - (6*in_list(free_elective[0], completed_courses, 0))
